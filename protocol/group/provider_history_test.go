...
func TestProviderReplacementRetestedAfterOldCheckCompletes(t *testing.T) {
	...
	history.AddProviderOutbound(previous)
	history.AddProviderOutbound(replacement)
	...
	g.storeOutbounds([]string{"node"}, []adapter.Outbound{previous})
	...
	history.RemoveProviderOutbound(previous)
	...
	require.Nil(t, history.LoadURLTestHistoryForOutbound(replacement))
	require.Nil(t, history.LoadURLTestHistoryForOutbound(previous))
