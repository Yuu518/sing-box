...
func (m *lifecycleTestProviderManager) RegisterCallback(callback adapter.ProviderUpdateCallback) *list.Element[adapter.ProviderUpdateCallback] {
	return m.callbacks.PushBack(callback)
}

func (m *lifecycleTestProviderManager) UnregisterCallback(element *list.Element[adapter.ProviderUpdateCallback]) {
...
func TestSelectorWaitsForProviderDefault(t *testing.T) {
	...
	require.Equal(t, "test-fallback", selector.Selected(N.NetworkTCP).Tag())
	provider.members = []adapter.Outbound{manager.outbounds["a"], manager.outbounds["b"]}
	require.NoError(t, selector.onProviderUpdated("p"))
	require.NoError(t, selector.PostStart())
	require.Equal(t, "b", selector.Selected(N.NetworkTCP).Tag())
}
...
func TestSelectorUsesAlreadyLoadedProvider(t *testing.T) {
	...
	require.NoError(t, selector.PostStart())
	require.Equal(t, "a", selector.Selected(N.NetworkTCP).Tag())
}
