...
func TestProviderGroupConcurrentUpdates(t *testing.T) {
	...
	t.Run("Selector", func(t *testing.T) {
		...
		runConcurrentProviderUpdates(t, selector.onProviderUpdated, func() {
			_ = selector.All()
			_ = selector.Selected(N.NetworkTCP)
			_ = selector.References()
	...
	t.Run("URLTest", func(t *testing.T) {
		...
		group.storeOutbounds([]string{firstOutbound.Tag(), secondOutbound.Tag()}, []adapter.Outbound{firstOutbound, secondOutbound})
		urlTest := &URLTest{
			outbound:       outboundManager,
			group:          group,
			providers:      providers,
...
func TestURLTestProviderUpdateReplacesSelectedOutboundInstance(t *testing.T) {
	...
	group.storeOutbounds([]string{previous.Tag()}, []adapter.Outbound{previous})
	group.selectedOutboundTCP.Store(previous)
	group.selectedOutboundUDP.Store(previous)

	group.replaceOutbounds([]string{replacement.Tag()}, []adapter.Outbound{replacement})
...
func (m *providerUpdateTestOutboundManager) ProviderFallback() (adapter.Outbound, error) {
...
func (o *providerUpdateTestOutbound) Tag() string { return o.tag }

func (o *providerUpdateTestOutbound) Network() []string { return []string{"tcp", "udp"} }
