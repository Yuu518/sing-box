...
func (s *URLTest) Start() error {
	...
	if len(s.providerTags) > 0 {
		_, members, cache, err := collectProviderOutbounds(s.providerTags[0], s.Dependencies(), s.outbound, s.providers, s.providerTags, s.outboundsCache, s.exclude, s.include)
		if err != nil {
			return err
		}
		outbounds = members
...
func (g *URLTestGroup) replaceOutbounds(outbounds []adapter.Outbound) {
	g.updateAccess.Lock()
	previousByTag := make(map[string]adapter.Outbound)
	for _, previous := range g.loadOutbounds() {
		previousByTag[previous.Tag()] = previous
	}
	for _, current := range outbounds {
		if previous := previousByTag[current.Tag()]; previous != nil && previous != current && g.history != nil {
			g.history.SetCurrentOutbound(current)
		}
	}
	selectedOutboundTCP := g.selectedOutboundTCP.Load()
	selectedOutboundUDP := g.selectedOutboundUDP.Load()
