...
func NewProviderInline(ctx context.Context, router adapter.Router, logFactory log.Factory, tag string, options option.ProviderInlineOptions) (adapter.Provider, error) {
	...
	provider := &ProviderLocal{
		Adapter: provider.NewAdapter(ctx, router, logFactory, logger, tag, C.ProviderTypeInline, options.HealthCheck),
		ctx:     ctx,
		logger:  logger,
	}
	provider.Adapter.Update(options.Outbounds, options.Endpoints)
	return provider, nil
}
...
func NewProviderLocal(ctx context.Context, router adapter.Router, logFactory log.Factory, tag string, options option.ProviderLocalOptions) (adapter.Provider, error) {
	...
	provider := &ProviderLocal{
		Adapter: provider.NewAdapter(ctx, router, logFactory, logger, tag, C.ProviderTypeLocal, options.HealthCheck),
		ctx:     ctx,
		logger:  logger,
