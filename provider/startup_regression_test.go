...
func TestRemoteProviderCanDownloadThroughEarlierProviderGroup(t *testing.T) {
	...
	require.NoError(t, err)
	var options option.Options
	config := fmt.Sprintf(`{"log":{"disabled":true},"outbounds":[{"type":"selector","tag":"download","providers":["local"]}],"outbound_providers":[{"type":"local","tag":"local","path":%s},{"type":"remote","tag":"remote","url":%q,"http_client":{"detour":"download"}}]}`, encodedPath, server.URL)
	require.NoError(t, json.UnmarshalContext(ctx, []byte(config), &options))
	instance, err := box.New(box.Options{Context: ctx, Options: options})
	require.NoError(t, err)
	defer func() { cancel(); _ = instance.Close() }()
...
func TestStaticOutboundCannotReferenceProviderNode(t *testing.T) {
	ctx, cancel := context.WithCancel(include.Context(context.Background()))
	defer cancel()
	path := filepath.Join(t.TempDir(), "provider.json")
	...
	require.NoError(t, err)
	defer func() { cancel(); _ = instance.Close() }()
	require.ErrorContains(t, instance.Start(), "dependency[node] not found")
}
