...
func (s *SavedBinary) MarshalBinary() ([]byte, error) {
	...
	err := binary.Write(&buffer, binary.BigEndian, uint8(3))
	if err != nil {
		return nil, err
	}
...
func (s *SavedBinary) UnmarshalBinary(data []byte) error {
	...
	if hashLength > uint64(reader.Len()) {
		return E.New("invalid hash length: ", hashLength)
	}
	...
	return s.Hash.UnmarshalBinary(hash)
}

type OutboundGroup interface {
	Outbound
	All() []string
	Member(tag string) (Outbound, bool)
	Selected(network string) Outbound
	SelectedTag(network string) string
