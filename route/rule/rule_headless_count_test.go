...
func TestHeadlessRuleEntryCount(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "single condition entries",
			options: option.DefaultHeadlessRule{
				ProcessName: []string{"a", "b", "c"},
			},
			count: 3,
		},
		{
			...
			options: option.DefaultHeadlessRule{
				ProcessName: []string{"a", "b"},
				Port:        []uint16{443},
			},
			count: 1,
		},
		{
			name: "conditions without items",
			options: option.DefaultHeadlessRule{
		...
		t.Run(fmt.Sprint(version), func(t *testing.T) {
			...
			for _, recover := range []bool{false, true} {
				t.Run(fmt.Sprint("recover=", recover), func(t *testing.T) {
					...
					require.NoError(t, err)
					if version == C.RuleSetVersion1 && !recover {
						require.Equal(t, expected+1, headlessRuleCount(rule))
					} else {
						require.Equal(t, expected, headlessRuleCount(rule))
					}
				})
			}
		})
...
func TestHeadlessRuleEntryCountMmapMatchers(t *testing.T) {
	...
	matcher, err := domain.NewMatcherFromMmap(domain.NewMatcher(
		[]string{"a.example", "b.example"}, []string{"example.org"}, false,
	).Mmap())
	require.NoError(t, err)
...
func TestHeadlessRuleEntryCountEmptyIPSet(t *testing.T) {
	...
	require.NoError(t, err)
	require.Zero(t, headlessRuleCount(rule))
}
