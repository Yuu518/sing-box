package rule

import (
	"context"
	"errors"
	"io"
	"io/fs"
	"path/filepath"
	"strings"
	"sync"
	"time"

	"github.com/sagernet/fswatch"
	"github.com/sagernet/sing-box/adapter"
	C "github.com/sagernet/sing-box/constant"
	"github.com/sagernet/sing-box/log"
	"github.com/sagernet/sing-box/option"
	"github.com/sagernet/sing/common"
	E "github.com/sagernet/sing/common/exceptions"
	"github.com/sagernet/sing/common/logger"
	"github.com/sagernet/sing/service/filemanager"
)

var _ adapter.RuleSet = (*LocalRuleSet)(nil)

type LocalRuleSet struct {
	abstractRuleSet
	watcher      *fswatch.Watcher
	reloadAccess sync.Mutex
}

func NewLocalRuleSet(ctx context.Context, logger logger.ContextLogger, tag string, options option.RuleSet) (*LocalRuleSet, error) {
	ruleSet := &LocalRuleSet{
		abstractRuleSet: abstractRuleSet{
			ctx:    ctx,
			logger: logger,
			tag:    tag,
			sType:  options.Type,
			format: options.Format,
		},
	}
	if options.Type == C.RuleSetTypeInline {
		if len(options.InlineOptions.Rules) == 0 {
			return nil, E.New("empty inline rule-set")
		}
		err := ruleSet.reloadRules(options.InlineOptions.Rules, ruleSet)
		if err != nil {
			return nil, err
		}
	} else {
		path, err := ruleSet.getPath(ctx, strings.ReplaceAll(options.Path, C.RuleSetTagPlaceholder, tag))
		if err != nil {
			return nil, err
		}
		ruleSet.path = path
		err = ruleSet.reloadFile(ctx, path)
		if err != nil {
			return nil, err
		}
		watcher, err := fswatch.NewWatcher(fswatch.Options{
			Path: []string{path},
			Callback: func(path string) {
				uErr := ruleSet.reloadFile(ctx, path)
				if uErr != nil {
					logger.ErrorContext(log.ContextWithNewID(context.Background()), E.Cause(uErr, "reload rule-set ", tag))
				}
			},
		})
		if err != nil {
			return nil, err
		}
		ruleSet.watcher = watcher
	}
	return ruleSet, nil
}

func (s *LocalRuleSet) StartContext(ctx context.Context, startContext *adapter.HTTPStartContext) error {
	if s.watcher != nil {
		err := s.watcher.Start()
		if err != nil {
			s.logger.Error(E.Cause(err, "watch rule-set file"))
		}
	}
	return nil
}

func (s *LocalRuleSet) reloadFile(ctx context.Context, path string) error {
	s.reloadAccess.Lock()
	defer s.reloadAccess.Unlock()
	if err := ctx.Err(); err != nil {
		return err
	}
	file, err := filemanager.Open(s.ctx, path)
	if err != nil {
		return err
	}
	defer file.Close()
	content, err := io.ReadAll(file)
	if err != nil {
		return err
	}
	rules, err := s.parseBytes(content)
	if err != nil {
		return err
	}
	info, err := file.Stat()
	if err != nil {
		return err
	}
	if err = ctx.Err(); err != nil {
		return err
	}
	updatedAt := time.Now()
	if s.lastUpdatedTime().IsZero() {
		updatedAt = info.ModTime()
	}
	err = s.applyRules(rules, s, updatedAt)
	if err != nil {
		return err
	}
	s.releaseUnreferencedRules()
	return nil
}

func (s *LocalRuleSet) getPath(ctx context.Context, path string) (string, error) {
	if path == "" {
		path = s.tag
		switch s.format {
		case C.RuleSetFormatSource, "":
			path += ".json"
		case C.RuleSetFormatBinary:
			path += ".srs"
		}
	}
	path = filemanager.BasePath(ctx, path)
	path, _ = filepath.Abs(path)
	info, err := filemanager.Stat(ctx, path)
	if err == nil && info.IsDir() {
		return "", E.New("rule_set path is a directory: ", path)
	}
	if err != nil && !errors.Is(err, fs.ErrNotExist) {
		return "", E.Cause(err, "check rule-set path")
	}
	return path, nil
}

func (s *LocalRuleSet) Update(ctx context.Context) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	if s.sType == C.RuleSetTypeInline {
		return nil
	}
	return s.reloadFile(ctx, s.path)
}

func (s *LocalRuleSet) Close() error {
	err := common.Close(common.PtrOrNil(s.watcher))
	s.reloadAccess.Lock()
	defer s.reloadAccess.Unlock()
	s.closeRules()
	return err
}
