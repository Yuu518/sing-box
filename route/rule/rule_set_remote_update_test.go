...
func TestRemoteRuleSetPersistenceFailureRecovery(t *testing.T) {
	...
	for _, failure := range []string{"file", "metadata", "rename", "database-only"} {
		t.Run(failure, func(t *testing.T) {
			...
			default:
				require.Equal(t, newContent, string(cache.LoadRuleSet("test").Content))
			}
			if path != "" {
				persisted, err := os.ReadFile(path)
				require.NoError(t, err)
				if failure == "metadata" {
					require.Equal(t, newContent, string(persisted))
				} else {
			...
			require.NoError(t, ruleSet.Update(t.Context()))
			require.Empty(t, requestETags[len(requestETags)-1])
			require.Equal(t, 3, callbacks)
			require.False(t, ruleSet.cacheDirty)
...
func TestRemoteRuleSetFallsBackFromMissingCacheFile(t *testing.T) {
	t.Parallel()
	path := filepath.Join(t.TempDir(), "rules.json")
	cache := new(ruleSetTestCache)
	...
	require.True(t, ruleSet.cacheDirty)
	require.True(t, ruleSet.Match(&adapter.InboundContext{Domain: "cached.example"}))
	require.True(t, lastUpdated.Equal(ruleSet.UpdatedTime()))
}

func TestRemoteRuleSetUpdatedTimeWithoutSourceTime(t *testing.T) {
	t.Parallel()
	ruleSet, err := NewRemoteRuleSet(t.Context(), logger.NOP(), "test", ruleSetTestOptions(""))
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, ruleSet.Close()) })
	require.True(t, ruleSet.UpdatedTime().IsZero())
	require.NoError(t, ruleSet.loadBytes([]byte(`{"version":4,"rules":[{"domain":["example.com"]}]}`), ruleSet))
	require.False(t, ruleSet.UpdatedTime().IsZero())
	require.True(t, ruleSet.lastUpdatedTime().IsZero())
}
...
func TestRemoteRuleSetNotModifiedMetadataFailure(t *testing.T) {
	...
	require.NoError(t, ruleSet.Update(t.Context()))
	require.False(t, ruleSet.UpdatedTime().Before(updatedAt))
	require.True(t, ruleSet.cacheDirty)
	cache.err = nil
	require.NoError(t, ruleSet.Update(t.Context()))
