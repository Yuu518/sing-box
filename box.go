...
func New(options Options) (*Box, error) {
	...
	outboundManager.InitializeProviderFallback(func(tag string) (adapter.Outbound, error) {
		return direct.NewOutbound(ctx, router, logFactory.NewLogger("outbound/direct["+tag+"]"), tag, option.DirectOutboundOptions{})
	})
	providerManager.Initialize(outboundManager.StartAvailable)
...
func (s *Box) preStart() error {
	...
	if len(s.provider.Providers()) > 0 {
		err = s.outbound.StartAvailable()
	} else {
	...
	err = adapter.Start(s.ctx, s.logger, adapter.StartStateStart, s.dnsTransport, s.network, s.connection)
	if err != nil {
		return err
	}
	...
	err = adapter.Start(s.ctx, s.logger, adapter.StartStateStart, s.provider)
	if err != nil {
		return err
	}
	if len(s.provider.Providers()) > 0 {
		err = s.outbound.StartRemaining()
		if err != nil {
			return err
		}
	}
...
func (s *Box) start() error {
	...
	err = adapter.Start(s.ctx, s.logger, adapter.StartStatePostStart, s.outbound, s.network, s.dnsTransport, s.dnsRouter, s.connection, s.router, s.endpoint, s.provider, s.certificateProvider, s.inbound, s.service)
	if err != nil {
		return err
	}
