...
func New(options Options) (*Box, error) {
	...
	outboundManager.InitializeProviderFallback(func(tag string) (adapter.Outbound, error) {
		bootstrap, err := direct.NewOutbound(ctx, router, logFactory.NewLogger("outbound/direct["+tag+"]"), tag, option.DirectOutboundOptions{})
		if err != nil {
			return nil, err
		}
		return outbound.NewProviderFallback(tag, bootstrap), nil
	})
