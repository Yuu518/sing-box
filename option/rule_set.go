...
func (r RuleSet) MarshalJSON() ([]byte, error) {
	if r.Type != C.RuleSetTypeInline {
		...
		case C.RuleSetTypeRemote:
			defaultFormat = remoteRuleSetDefaultFormat(r.RemoteOptions.URL, r.Path)
		}
...
func (r *RuleSet) UnmarshalJSON(bytes []byte) error {
	...
	if r.Type != C.RuleSetTypeInline {
		if r.Format == "" {
			...
			case C.RuleSetTypeRemote:
				r.Format = remoteRuleSetDefaultFormat(r.RemoteOptions.URL, r.Path)
			}
		}
		switch r.Format {
		case "":
...
func remoteRuleSetDefaultFormat(url, path string) string {
	if format := ruleSetDefaultFormat(url); format != "" {
		return format
	}
	return ruleSetDefaultFormat(path)
}
